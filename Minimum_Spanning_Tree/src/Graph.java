...
{
    public Vertex     src;    // First vertex in Edge(so we can keep track the first vertex of an edge)
...
public class Graph
{
    ...
    /**
     * Using PriorityQueue construct a tree that connect all vertex and has the smallest weight.
     */
    public void prim ( String startName ) {
        ...
        //first pick the start point
        v = getVertex (startName);
        //then remove the start point from vertexSet(vertexSet only have the point that in vertexSet and didn't in mst)
        vertexSet.remove(v);
        for( Edge e : v.adj )
            pq.add(e);
        //pick from the adjacent vertex as eg
        while (!vertexSet.isEmpty()) {
            eg = pq.remove();

            //while eg's second element is not in vertexSet,(every adjacent of eg have been romove before) then remove new element from pg again
            while ( !vertexSet.contains(eg.dest))
                eg = pq.remove();
            mst.addEdge(eg.src.name, eg.dest.name, eg.cost);

            // update mst cost
            mst.cost += eg.cost;

            // add candidate edges to priority queue
            for ( Edge e: eg.dest.adj) {
                if ( vertexSet.contains(e.dest) ) {
                    pq.add(e);
                }
            }
            vertexSet.remove(eg.dest);
        }
        mst.printGraph();
    }
